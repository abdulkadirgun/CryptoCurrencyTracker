...
class NotificationWorker @AssistedInject constructor(
    ...
    private val getCoinFromRemoteSaveToDB: GetCoinsAndSaveDBUseCase
    ) : CoroutineWorker(context, workerParams)
{
    override suspend fun doWork(): Result {
        Log.d("NotificationService", "doWork called")
        ...
        * */
        getFavCoinsUseCase().collect{ coins->
                when(coins){
                    ...
                    is Resource.Loading -> {}
                    is Resource.Success -> {
                        Log.d("NotificationService", "coins: ${coins.data}")

                        val priceChangedCoinList = arrayListOf<String>()

                        coins.data?.forEach { coinOld ->
                            getCoinByIdUseCase(coinId = coinOld.id).collect{ coinNew->
                                when(coinNew){
                                    is Resource.Error -> {}
                                    is Resource.Loading -> {}
                                    is Resource.Success -> {
                                        if (coinNew.data?.market_data?.current_price?.usd?.roundToTwoDecimal() != coinOld.current_price){
                                            priceChangedCoinList.add(coinOld.symbol)
                                            Log.d("NotificationService", "coin eklendi eski:${coinOld.current_price} yeni:${coinNew.data?.market_data?.current_price?.usd}")
                                        }
                                        else
                                            Log.d("NotificationService", "coin eklenmedi $coinOld")
                                    }
                                }
                            }
                        }
                        ...
                        if (priceChangedCoinList.isNotEmpty()) {
                            ...
                            Log.d("NotificationService", "favori coinlerin fiyatı değişmemiş")
                    }
                }
            }
